...
public class FakeRepository implements IBarRepository {
    ...
    public List<Bar> getFilteredBars(String city, List<String> tags, double minRating) {
        ...
        if (tags != null && !tags.isEmpty()) {
            selectedType = tags.get(0);
        }

        for (Bar bar : ALL_BARS) {
            if (city != null && !city.isEmpty() && !bar.getCity().equalsIgnoreCase(city)) {
                continue;
            }

            if (minRating > 0 && bar.getRating() < minRating) {
                continue;
            }

            if (selectedType != null
                    && !selectedType.equalsIgnoreCase("All")
                    && (bar.getType() == null || !bar.getType().equalsIgnoreCase(selectedType))) {
                continue;
            }
